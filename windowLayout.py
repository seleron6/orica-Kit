    ...
    layout=[
        [
            sg.Frame("SQLから生成",
                [
                    [
                        sg.Text("datas",size=(4,1)),
                        sg.Input(key="-DATAS_SQL-",expand_x=True)
                    ],
                    [
                        sg.Text("texts",size=(4,1)),
                        sg.Input(key="-TEXTS_SQL-",expand_x=True)
                    ],
                    [
                        sg.Button("生成（未完成のため無効化中）",key="-CREATE_FROM_SQL-",expand_x=True,disabled=True)
                    ]
                ],expand_x=True
            )
        ]
